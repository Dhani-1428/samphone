  ...
  return (
    <div className="max-w-7xl relative mx-auto py-20 md:py-32 px-4 md:px-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-foreground/50 text-sm uppercase tracking-widest">Premium Brands Collection</span>
      </div>
      <h1 className="text-5xl md:text-7xl font-display font-bold text-foreground mb-4">Multi Brand Store</h1>
      <p className="text-foreground/60 text-xl max-w-xl">Top international brands, all in one place. Scroll to explore all brands.</p>
    </div>
  );
}
...
export default function MultiBrand() {
  ...
  return (
    <div className="bg-background">
      <HeroParallax brands={allBrands} header={<MultiBrandHeader />} compact />
      ...
      <div className="container mx-auto px-4 md:px-6 py-10">
        <div className="grid grid-cols-3 gap-4 mb-10">
          {[{ icon: Award, value: "20+", label: "Premium Brands" }, { icon: TrendingUp, value: "500+", label: "Brand Products" }, { icon: Star, value: "4.8/5", label: "Average Rating" }].map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-card border border-border rounded-2xl p-4 text-center">
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-10">
          {brandCards.map((b) => (
            <motion.button key={b.name} variants={itemVariants} whileHover={{ scale: 1.05 }} onClick={() => setSelectedBrand(selectedBrand === b.name ? null : b.name)} className={`rounded-2xl p-4 border-2 transition-all text-left ${selectedBrand === b.name ? "border-primary shadow-lg" : "border-border hover:border-primary/50"}`}>
        ...
        <h2 className="text-xl font-display font-bold text-foreground mb-5">{selectedBrand ? `${selectedBrand} Products` : "Featured Products"}</h2>
        <motion.div ref={ref} variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-5">
          {visible.map((p) => (
            <motion.div key={p.id} variants={itemVariants}><ProductCard {...p} testPrefix="multi" /></motion.div>
          ))}
        </motion.div>
