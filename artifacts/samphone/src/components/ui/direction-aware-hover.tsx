...
export const DirectionAwareHover = ({
  ...
  return (
    <motion.div
      ...
      ref={ref}
      className={cn("bg-transparent rounded-none overflow-hidden group/card relative", className)}
    >
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <motion.div
            variants={variants}
            className="h-full w-full relative bg-muted"
            transition={{ duration: 0.2, ease: "easeOut" }}
          >
            <img
              alt="product"
              className={cn("h-full w-full object-cover scale-[1.12]", imageClassName)}
              src={imageUrl}
            />
          </motion.div>
          <motion.div
            ...
            className={cn("text-white absolute bottom-3 left-3 z-40", childrenClassName)}
          >
            {children}
          </motion.div>
        </motion.div>
