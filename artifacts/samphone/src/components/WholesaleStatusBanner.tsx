...
export default function WholesaleStatusBanner() {
  ...
  if (seesWholesalePrices(user)) {
    ...
    const discount =
      pct != null && pct > 0 ? t("wholesale_discount_suffix", { percent: String(pct) }) : "";
    return (
      <div className="bg-[#F3F3F3] px-4 py-2 text-center text-sm text-[#111111]">
        {t("wholesale_approved_banner", { discount })}
      </div>
    );
  }
  ...
  const key =
    status === "rejected"
      ? "wholesale_rejected_banner"
      : status === "suspended"
        ? "wholesale_suspended_banner"
        : "wholesale_pending_banner";
  return (
